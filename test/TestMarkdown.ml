...
let aeq_pars ?msg expected actual = assert_equal ?msg expected actual

let check expected input =
  aeq_pars
    ~msg:(sprintf "With input:\n%s\n" (BatString.strip input))
...
let test_read_list _test_ctxt =
  check
    [
      Ulist
        ([ Normal [ Text "foo "; Bold "bar" ] ], [ [ Normal [ Text "baz" ] ] ]);
    ]
    "* foo\n*bar*\n* baz";
  check
    [
  ...
  check
    [
      Ulist
        ( [ Normal [ Text "foo" ]; Normal [ Text "bar" ] ],
          [ [ Normal [ Text "baz" ] ] ] );
    ]
    "* foo\n\n bar\n* baz";
  check [ Ulist ([ Normal [ Text "foo" ] ], []) ] "* foo";
  check
  ...
  check
    [
      Ulist
        ( [
            Normal [ Text "some paragraph" ];
            Normal [ Text "And another one." ];
          ],
  ...
  check
    [
      Ulist
        ( [ Normal [ Text "foo "; Bold "bar baz" ]; Normal [ Text "xxx" ] ],
          [ [ Normal [ Text "baz" ] ] ] );
    ]
    "*\tfoo\n*bar\n baz*\n\n xxx\n\n* baz";
  check
...
let test_read_normal _test_ctxt =
  check
    [ Normal [ Text "foo "; Struck [ Text " bar baz " ]; Text " foobar" ] ]
    "foo == bar\nbaz == foobar";
  check
    [
      Normal
...
let test_read_normal_unmatched _test_ctxt =
  ...
  check [ Normal [ Text "foo == bar" ] ] "foo == bar";
  check
    [ Normal [ Text "foo == bar" ]; Normal [ Text "baz ==" ] ]
    "foo == bar\n\nbaz =="
...
let test_heading _test_ctxt =
  for i = 1 to 6 do
    check
      [
        Heading
          (i, [ Text "foo "; Link { href_target = "dst"; href_desc = "foo" } ]);
      ]
  ...
  check
    [
      Quote
        [
          Ulist
            ( [ Normal [ Text "one" ]; Normal [ Text "xxx" ] ],
              [ [ Normal [ Text "two" ] ] ] );
        ];
    ]
    "> * one\n>\n>   xxx\n> * two\n\n"
  ...
  check
    [
        ...
        [
          Text
            "OASIS generates a full configure, build and install system for \
             your application. It starts with a simple ";
          Code "_oasis";
          Text
            " file at the toplevel of your project and creates everything \
             required.";
        ];
      Normal
        [
          Text
            "It uses external tools like OCamlbuild and it can be considered \
      ...
      Normal
        [
          Text
            "It also allows to have standard entry points and description. It \
             helps to integrates your libraries and software with third \
    ...
    "OASIS generates a full configure, build and install system for your\n\
     ...
     glue\n\
     between various subsystems that do the job. It should support the \
     following\n\
     tools:\n\n\
     - OCamlbuild\n\
     - OMake (todo)\n\
